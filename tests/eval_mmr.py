#!/usr/bin/env python3
"""
Variant 4: MMR - Hybrid retrieval + MMR reranking

No graph features.

Usage:
    python3 tests/eval_mmr.py [--num-books 200] [--output-dir ./results]
"""
...
def evaluate_mmr(num_books: int = 200, output_dir: Path = None, processed_dir: Path = None) -> Dict:
    """Evaluate MMR variant."""
    ...
    print(f"\n{'='*80}")
    print(f"🔬 VARIANT: MMR (Hybrid + Reranking)")
    print(f"{'='*80}")
    print(f"Config: use_dense=True, use_bm25=True, use_mmr=True, use_graph=False")
    ...
    # Save results
    pred_file = output_dir / 'predictions_mmr.json'
    with open(pred_file, 'w') as f:
        json.dump(all_predictions, f, indent=2)

    metrics_file = output_dir / 'metrics_mmr.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate MMR variant')
    parser.add_argument('--num-books', type=int, default=200, help='Number of books (default: 200)')
    parser.add_argument('--output-dir', type=Path, default=None, help='Output directory')
    args = parser.parse_args()

    evaluate_mmr(num_books=args.num_books, output_dir=args.output_dir)
