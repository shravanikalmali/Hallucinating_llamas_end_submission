#!/usr/bin/env python3
"""
Variant 3: Hybrid - Dense + BM25 combined retrieval

No MMR, no graph features.

Usage:
    python3 tests/eval_hybrid.py [--num-books 200] [--output-dir ./results]
"""
...
def evaluate_hybrid(num_books: int = 200, output_dir: Path = None, processed_dir: Path = None) -> Dict:
    ...
    print(f"\n{'='*80}")
    print(f"🔬 VARIANT: Hybrid (Dense + BM25)")
    print(f"{'='*80}")
    print(f"Config: use_dense=True, use_bm25=True, use_mmr=False, use_graph=False")
    ...
    # Save results
    pred_file = output_dir / 'predictions_hybrid.json'
    with open(pred_file, 'w') as f:
        json.dump(all_predictions, f, indent=2)

    metrics_file = output_dir / 'metrics_hybrid.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate Hybrid variant')
    parser.add_argument('--num-books', type=int, default=200, help='Number of books (default: 200)')
    parser.add_argument('--output-dir', type=Path, default=None, help='Output directory')
    args = parser.parse_args()

    evaluate_hybrid(num_books=args.num_books, output_dir=args.output_dir)
