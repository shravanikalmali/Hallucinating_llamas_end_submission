#!/usr/bin/env python3
"""
Variant 2: BM25 Only - Keyword-based retrieval only

No dense, no MMR, no graph features.

Usage:
    python3 tests/eval_bm25_only.py [--num-books 200] [--output-dir ./results]
"""
...
def evaluate_bm25_only(num_books: int = 200, output_dir: Path = None, processed_dir: Path = None) -> Dict:
    """Evaluate BM25 Only variant."""
    ...
    print(f"\n{'='*80}")
    print(f"🔬 VARIANT: BM25 Only")
    print(f"{'='*80}")
    print(f"Config: use_dense=False, use_bm25=True, use_mmr=False, use_graph=False")
    ...
    # Save results
    pred_file = output_dir / 'predictions_bm25_only.json'
    with open(pred_file, 'w') as f:
        json.dump(all_predictions, f, indent=2)

    metrics_file = output_dir / 'metrics_bm25_only.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate BM25 Only variant')
    parser.add_argument('--num-books', type=int, default=200, help='Number of books (default: 200)')
    parser.add_argument('--output-dir', type=Path, default=None, help='Output directory')
    args = parser.parse_args()

    evaluate_bm25_only(num_books=args.num_books, output_dir=args.output_dir)
