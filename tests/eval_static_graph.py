#!/usr/bin/env python3
"""
Variant 5: Static Graph - Full pipeline with graph features

Dense + BM25 + MMR + Graph expansion + Graph reasoning.

Usage:
    python3 tests/eval_static_graph.py [--num-books 200] [--output-dir ./results]
"""
...
def evaluate_static_graph(num_books: int = 200, output_dir: Path = None, processed_dir: Path = None) -> Dict:
    """Evaluate Static Graph variant."""
    ...
    print(f"\n{'='*80}")
    print(f"🔬 VARIANT: Static Graph (Full Pipeline)")
    print(f"{'='*80}")
    print(f"Config: use_dense=True, use_bm25=True, use_mmr=True, use_graph=True")
    print(f"Books: {num_books}")
    print(f"{'='*80}\n")

    # Set configuration - ALL GRAPH FEATURES ENABLED
    set_config({
        'use_dense': True,
        'use_bm25': True,
        'use_mmr': True,
        'use_graph_expansion': True,      # Static graph expansion
        'use_graph_reasoning': True,      # Dynamic graph reasoning
        'use_neighbors': True,            # Neighbor-based inclusion
    ...
    # Save results
    pred_file = output_dir / 'predictions_static_graph.json'
    with open(pred_file, 'w') as f:
        json.dump(all_predictions, f, indent=2)

    metrics_file = output_dir / 'metrics_static_graph.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate Static Graph variant')
    parser.add_argument('--num-books', type=int, default=200, help='Number of books (default: 200)')
    parser.add_argument('--output-dir', type=Path, default=None, help='Output directory')
    args = parser.parse_args()

    evaluate_static_graph(num_books=args.num_books, output_dir=args.output_dir)
